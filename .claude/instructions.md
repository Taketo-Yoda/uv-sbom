...
**コーディング完了時の必須チェック**:
1. **フォーマット**: `cargo fmt --all -- --check` がパスすること
2. **Clippy**: `cargo clippy --all-targets --all-features -- -D warnings` で警告ゼロ
3. **テスト**: `cargo test` が全テストパス
4. **テストカバレッジ**: 新機能には必ずテストを追加
5. **ドキュメント**: 公開APIには必ずドキュメントコメント
